    ...
    public partial class Form1 : Form {
        ...
        void VtoH() {
            foreach (string key in fovWithOptics.Keys) {
                fovWithOptics[key] = 2 * Math.Atan(Math.Tan(fovWithOptics[key] * hipfov / 2 * Math.PI / 180)
                    * aratio) / Math.PI * 180;
            }
...
#pragma warning disable CS8602 // null 参照の可能性があるものの逆参照です。-> 先にインデックスで判定してるからnullは来ない
...
#pragma warning restore CS8602 // null 参照の可能性があるものの逆参照です。
            ...
            if (mdratio > 100 || mdratio < 0) {
                MessageBox.Show("Monitar Distanceは0から100の割合で入力してください。", "エラー",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            if (hipdist < 0) {
                MessageBox.Show("振り向きは自然数で入力してください。", "エラー",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            switch (gameidx) {
            case 0:
                if (hipfov < 1 || hipfov > 2) {
                    MessageBox.Show("In-Game FOVは1から2のfovScaleで入力してください。", "エラー",
                        MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                fovWithOptics.Clear();
            ...
            case 1:
                if (hipfov < 60 || hipfov > 90) {
                    MessageBox.Show("In-Game FOVは60から90で入力してください。", "エラー",
                        MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                fovWithOptics.Clear();
            ...
            case 3:
                if (hipfov < 0 || hipfov > 180) {
                    MessageBox.Show("In-Game FOVは180未満の自然数で入力してください。", "エラー",
                        MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                fovWithOptics.Clear();
                ...
                if (aridx != 0) {
                    ...
                    foreach (string key in fovWithOptics.Keys) {
                        fovWithOptics[key] = 2 * Math.Atan(Math.Tan(fovWithOptics[key] / 2 * Math.PI / 180)
                            * aratio * 9 / 16) / Math.PI * 180;
                    }
                }
                calcSens();
                break;
            ...
            case 5:
                if (hipfov < 80 || hipfov > 103) {
                    MessageBox.Show("In-Game FOVは80から103で入力してください。", "エラー",
                        MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                fovWithOptics.Clear();
